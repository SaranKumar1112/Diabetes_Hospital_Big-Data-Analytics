...
def update_time_in_hospital_plot(selected_variable):
    ...
    fig.update_layout(xaxis_title='Time in Hospital', yaxis_title='Frequency', bargap=0.3, title_x=0.5)  # Center-align title
    return fig
...
def update_selected_variable_plot(selected_variable):
    ...
    fig = px.histogram(df, x=selected_variable, nbins=30, title=f'Distribution of {selected_variable}', color_discrete_sequence=color)
    # Add space between the bars using the bargap parameter
    fig.update_layout(xaxis_title=selected_variable.replace('_', ' ').title(), yaxis_title='Frequency', bargap=0.3, title_x=0.5)  # Center-align title
   
    return fig, f"Selected Variable: {selected_variable.replace('_', ' ').title()}"
 
# Define the callback for the readmission plot
@app.callback(
